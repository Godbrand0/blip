...
var onHttpTrigger = async (runtime2, payload) => {
  try {
    ...
    try {
      const worldIdResponse = await fetch(runtime2.config.worldIdVerifyUrl, {
        method: "POST",
      ...
      if (!worldIdResult.success) {
        runtime2.log(`[ChainBridge] World ID verification failed`);
      ...
      runtime2.log(`[ChainBridge] World ID verification error: ${error.message}`);
      return JSON.stringify({ success: false, error: "System verification error" });
    }
