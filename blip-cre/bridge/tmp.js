...
var configSchema = exports_external.object({
  authorizedEVMAddress: exports_external.string(),
...
var onHttpTrigger = async (runtime2, payload) => {
  try {
    ...
    runtime2.log(`[ChainBridge] Processing execution for Intent: ${data.intentId}`);
    runtime2.log(`[ChainBridge] User: ${data.user}, Amount: ${data.amount}`);
    runtime2.log(`[ChainBridge] Verifying World ID proof...`);
    const httpClient = new ClientCapability2;
    const verifyBody = new TextEncoder().encode(JSON.stringify(data.proof));
    const verifyWorldId = (sendRequester, config) => {
    ...
    const worldIdResult = httpClient.sendRequest(runtime2, verifyWorldId, consensusIdenticalAggregation())(runtime2.config).result();
    if (!worldIdResult.success) {
      runtime2.log(`[ChainBridge] World ID verification failed: ${worldIdResult.error}`);
      return JSON.stringify({
        success: false,
        error: worldIdResult.error || "Identity verification failed"
      });
    }
    runtime2.log(`[ChainBridge] World ID verified successfully`);
    runtime2.log(`[ChainBridge] Finalizing bridge intent ${data.intentId} on World Chain...`);
    ...
    const bridgeResult = evmClient.sendTransaction(runtime2, {
      to: runtime2.config.vaultAddress,
      data: callData
    }).result();
    ...
    runtime2.log(`[ChainBridge] Error: ${error.message}`);
